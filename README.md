...
Динамическая документация рекомендована для работы в режиме разработки, она будет меняться по ходу добавления новых эндопинтов, изменений модели, изменений функциональности. Для просмотра динамической API документации необходимо предварительно [запустить сервер](#инструкция-запуска-сервера-локально). Далее документация будет доступна в удобно читаемом формате по эндпоинту http://127.0.0.1:8000/api/redoc/ .
* ### <a>Инструкция по работе с API</a>
Инструкция на примере создания папки, используется динамическая API.
![image](https://github.com/Mans-Mans/hw_python_oop/assets/118307945/faf2ad4c-0208-4478-afbc-6e64f0c42d28)
* В левой верхней части имеется название метода и его описание:
```
  Создание папки.
По эндпоинту "/api/folder/create" папка создаётся без расположения в папке, то есть на главной странице пользователя.
По эндпоинту "/api/folder/create-in-folder/{id}" новая папка создаётся в папке. В пути запроса указывается {id} папки, в которой создаётся новая папка.
Доступ: все авторизованные пользователи.
```
* В левой средней части мы видим:
  * Тип авторизации(Authorizations)
  * Параметры пути(Path parameters), которые должны быть указаны в пути запроса. А также тип и описание этих параметров:
```
  id               string <uuid>  A UUID string identifying this Папка.
required
```
  * Схему тела запроса, а так же то, в каком формате должны быть переданы параметры:
```
REQUEST BODY SCHEMA: application/json

 name                string (Название папки) [ 1 .. 255 ] characters
required
location	            string or null <uuid> (Расположение папки)
```
Параметры, под которыми красным написано __required__, должны быть переданы __обязательно__. Остальные параметры необязательны.
* В левой нижней части мы видим препдологаемый ответ от сервера:
  * Статус HTTP ответа
```
201
```
  * Схему ответа, то в каком формате должны вернуться параметры.
```
RESPONSE SCHEMA: application/json
```
  * Параметры и их тип, которые должен вернуть нам сервер.
```
id	                  string <uuid> (UUID)
name                 string (Название папки) [ 1 .. 255 ] characters
required
location	            string or null <uuid> (Расположение папки)
documents	           Array of strings <uuid> (Документы) unique
```
Параметры, под которыми красным написано __required__, не могут вернуться __пустыми__. Остальные параметры могут быть пустыми.
* В правой верхней части мы видим эндпоинт нашего запроса:
```
http://127.0.0.1:8000/api/folder/create-in-folder/{id}
```
* В правой средней части мы видим пример того, каким должен быть правильный запрос:
```
{
"name": "string",
...
```
* В правой нижней части мы видим пример того, каким должен быть правильный ответ от сервера:
  * Статус ответа и в каком формате вернулся ответ.
```
{
